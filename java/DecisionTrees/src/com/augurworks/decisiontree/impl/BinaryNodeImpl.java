...
public class BinaryNodeImpl<InputType extends Copyable<InputType>,
    ...
    public BinaryNodeImpl(ReturnType defaultLeft,
                          ReturnType defaultRight,
    ...
    @Override
    public String toFancyString(int indents) {
        StringBuilder sb = new StringBuilder();
        sb.append(indents(indents)).append("if ").append(typeOperatorLimit.toString()).append(":\n");
        if (leftHandChild == null) {
            sb.append(indents(indents + 1)).append(defaultLeft).append("\n");
        } else {
            sb.append(leftHandChild.toFancyString(indents + 1));
        }
        sb.append(indents(indents)).append("else:\n");
        if (rightHandChild == null) {
            sb.append(indents(indents + 1)).append(defaultRight).append("\n");
        } else {
            sb.append(rightHandChild.toFancyString(indents + 1));
        }
        return sb.toString();
    }
