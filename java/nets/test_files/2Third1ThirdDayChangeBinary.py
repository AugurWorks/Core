...
def scale(value,mx,mn,maxNum,minNum):
    return (value-mn)/float((mx-mn))*(maxNum-minNum)+minNum

readFileName='USO.csv'
...
daysBack=4 #Number of additional days used as inputs for stock data
...
writer.write('net '+str(numStocks*daysBack*2)+','+str(arrayDepth)+',1,-1,'+str(maxNum)+','+str(minNum)+'\n')
writer2.write('net '+str(numStocks*daysBack*2)+','+str(arrayDepth)+',1,-1,'+str(maxNum)+','+str(minNum)+'\n')
...
for r in range(daysAhead,numRows-varDays-daysBack):
    string=''
    if change[0][r-daysAhead]>=close[0][r-daysAhead]*cutoffChange:
        string+=str(scale(1,1,-1,maxNum,minNum))+' '
    elif change[0][r-daysAhead]<=-close[0][r-daysAhead]*cutoffChange:
        string+=str(scale(-1,1,-1,maxNum,minNum))+' '
    else:
        string+=str(scale(0,1,-1,maxNum,minNum))+' '
    for j in range(daysBack):
        for i in range(numStocks):
